...
private _aimingAccuracy = getMissionConfigValue ["AIMING_ACCURACY", 0.17];
private _aimingShake = getMissionConfigValue ["AIMING_SHAKE", 0.2];
private _aimingSpeed = getMissionConfigValue ["AIMING_SPEED", 0.1];
private _spotDistance = getMissionConfigValue ["SPOT_DISTANCE", 0.3];
private _spotTime = getMissionConfigValue ["SPOT_TIME", 0.3];
private _courage = getMissionConfigValue ["COURAGE", 0.25];
private _reloadSpeed = getMissionConfigValue ["RELOAD_SPEED", 0.25];
private _commanding = getMissionConfigValue ["COMMANDING", 0.80];
