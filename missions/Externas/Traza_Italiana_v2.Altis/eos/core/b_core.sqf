...
_Placement=(_mkrX + 500);
	...
	for "_counter" from 1 to _LVehGroups do {
		...
		_bnewpos = [_mPos, _Place, _dir_atk] call BIS_fnc_relPos;

		while {(surfaceiswater _bnewpos)} do {
			_dir_atk=_mkrAgl+(random _angle)-_angle/2;
			_pos = [_mPos, _Place, _dir_atk] call BIS_fnc_relPos;
			//PLAYER SIDECHAT (format ["%1",surfaceiswater _pos]);
		};
		...
		_bGroup=[_bnewpos,_side,_faction,_vehType]call EOS_fnc_spawnvehicle;
...
//*/
	...
	{
		/*systemChat format ["_pos: %1, _mPos: %2", _pos, _mPos];
		//_getToMarker = (_x select 2) addWaypoint [_pos, 0];
		...
		_wp setWaypointFormation "NO CHANGE";*/
		_dir_atk= _mPos getDir (_x select 0);
		_pos = [_mPos, (_mkrX + random 200), _dir_atk] call BIS_fnc_relPos;
		_group = (_x select 2);
		_leader = leader _group;
		_leader doMove _pos;
		//_group setVariable ["Vcm_Disable",true];
		
		//[_group, [22985.8,18515.3,0]] call BIS_fnc_taskAttack;
		/*_getToMarker = _group addWaypoint [_mPos, 0];
		_getToMarker setWaypointType "MOVE";
		_getToMarker setWaypointSpeed "FULL";
		_getToMarker setWaypointBehaviour "CARELESS";
		_getToMarker setWaypointFormation (["STAG COLUMN", "WEDGE", "ECH LEFT", "ECH RIGHT", "VEE", "DIAMOND", "LINE"] call BIS_fnc_selectRandom);
		_getToMarker setWaypointCompletionRadius 15;*/
			//sleep 1;
	}foreach _cGrp;
...
//player sidechat format ["Wave:%1 Condicion %2",_waves,(triggeractivated _bastActive and triggeractivated _bastClear)];
	if (triggeractivated _bastActive and triggeractivated _bastClear and (_waves < 1) ) then{
		...
		if (_waves >= 1) then {
			...
			// Modificado por Vultur. Rutina de Ignacio
			{
				if(_x getVariable "ACE_isUnconscious" isEqualTo true && !isPlayer _x && (side _x == _lado)) then {
					_x setDamage 1;
				};
			}forEach allUnits;
			////////////////////////////
