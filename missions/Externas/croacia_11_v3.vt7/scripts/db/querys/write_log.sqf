...
if (_logType == "connected") then {
	...
	_query = "INSERT INTO log (`log_type_id`, `player_name`, `player_uid`, `player_state_id`, `mission_name`, `role_id`) VALUES";

	_values = format [
		"((SELECT id from log_type WHERE name = '%1'), '%2', '%3', (SELECT id from player_state WHERE name = '%4'), '%5', (SELECT id from role WHERE code = '%6'));",
		_logType,
		name _unit,
		getPlayerUID _unit,
