...
OBTENER_SERIE =
{
	hint "Numro de serie: 4JFY3L";
};

OBTENER_RECUPERACION =
{
	hint "Código de ru de serie: 8510672";
};
...
CORTAR_CABLES =
{
	...
	if (!MANDI_DESACTIVACION_INICIADA) then {
		...
		[(time + 1200), "scripts\loop_bomb.sqf"] remoteExec ["BIS_fnc_execVM", 2, true];
	};
};
...
DETONATE_BOMB =
{
	[[], "scripts\detonate_bomb.sqf"] remoteExec ["BIS_fnc_execVM", 2, true];
};
