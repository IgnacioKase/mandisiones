/*******************************************************************************
                          Realizado por |ArgA|Ignacio
*******************************************************************************/

author = "";                            // Editor
onLoadName = "";	                    // Nombre de carga
briefingName = "";                      // Nombre en briefing
onLoadMission = "";                     // Pie de carga
overviewText = "";                      // Pie en seleccion de mision
overviewPicture = "imgs\portada.jpg";   // Imagen en seleccion de mision
loadScreen = "imgs\portada.jpg";        // Imagen en carga
IMG_LOGO = "imgs\logo.paa"              // Nota: Las imagenes deberian estar en
COLOR_AUTOR = "#76a8f7";                // formato .paa y en proporcion 2x1 (1024x512)
ES_CAMPANIA = 0;                        // tamaño logo: 323px x 323px
NOMBRE_CAMPANIA = "";

MAX_DIST_VISION = 2500;
GAFAS_INICIALES = "";
DESACTIVAR_EQUIPAMIENTO_PERSONALIZADO = 1;

//BRIEFING
CARACTER = "No Oficial";
HORARIO = "";
FECHA = "";
CLIMA = "";
UBICACION = "";
SITUACION = [""];               //Cada elemento del array es un parrafo
SITUACION_HISTORICA = [""];     //Ejemplo: ["Primer parrafo", "Segundo parrafo"]
EXTRACTO_INTELIGENCIA = [""];
OBJETIVOS = ["", ""];
RECURSOS = ["", ""];
ENEMIGOS = ["", ""];
////////////////////////////////////////////////////////////////////////////////

//IA - CONFIGURACION
DESACTIVAR_IA_DE_GRUPO = 1;
DIFICULTAD_IA_PERSONALIZADA = 1;
AIMING_ACCURACY = 0.01; //precision de apuntado
AIMING_SHAKE = 0.99; 	//dispersión de apuntado
AIMING_SPEED = 0.1; 	//velocidad de apuntado
SPOT_DISTANCE = 0.2; 	//distancia de avistamiento
SPOT_TIME = 0.5; 		//tiempo de avistamiento
COURAGE = 0.25; 		//coraje
RELOAD_SPEED = 0.25; 	//velocidad de recarga
COMMANDING = 0.75; 		//liderazgo
////////////////////////////////////////////////////////////////////////////////

/*******************************************************************************
                          Realizado por |ArgA|Ignacio
*******************************************************************************/